...
echo "[info] Active providers: embedding=${ASSISTANT_EMBEDDING_PROVIDER:-tei} reranker=${ASSISTANT_RERANKER_PROVIDER:-tei} answers=${AI_ANSWER_PROVIDER:-ollama}"
...
if [[ "${ASSISTANT_EMBEDDING_PROVIDER:-tei}" == "gemini" || "${ASSISTANT_RERANKER_PROVIDER:-tei}" == "gemini" || "${AI_ANSWER_PROVIDER:-ollama}" == "gemini" ]]; then
  if [[ -z "${GEMINI_API_KEY:-}" ]]; then
    echo "[warn] GEMINI_API_KEY is not set; Gemini provider(s) will fail at startup validation"
  else
...
if [[ "${ASSISTANT_EMBEDDING_PROVIDER:-tei}" == "tei" ]]; then
  echo "  embeddings:  tei -> http://localhost:${ASSISTANT_EMBEDDING_TEI_PORT:-8091}"
else
  echo "  embeddings:  ${ASSISTANT_EMBEDDING_PROVIDER:-tei} (cloud)"
fi
if [[ "${ASSISTANT_RERANKER_PROVIDER:-tei}" == "tei" ]]; then
  echo "  reranker:    tei -> ${ASSISTANT_RERANKER_TEI_URL:-http://localhost:8092/rerank}"
else
  echo "  reranker:    ${ASSISTANT_RERANKER_PROVIDER:-tei} (cloud)"
fi
if [[ "${AI_ANSWER_PROVIDER:-ollama}" == "ollama" || "${AI_ANSWER_PROVIDER:-ollama}" == "local-llm" ]]; then
  echo "  answers:     ${AI_ANSWER_PROVIDER:-ollama} -> ${AI_ANSWER_OLLAMA_URL:-http://localhost:11434/api/generate}"
else
  echo "  answers:     ${AI_ANSWER_PROVIDER:-ollama} (cloud)"
fi
